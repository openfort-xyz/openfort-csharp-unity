...
namespace Openfort.OpenfortSDK.Editor
{
    internal class IdentityPostprocess : IPostprocessBuildWithReport
    {
        ...
        {
            Debug.Log($"OpenfortSDK post-processing...");
            ...
            if (buildTarget == BuildTarget.StandaloneWindows64 || buildTarget == BuildTarget.StandaloneOSX || buildTarget == BuildTarget.iOS)
            {
                ...
                Debug.Log($"Successfully copied OpenfortSDK files");
            }
        ...
        private void CopyIntoDataDir(string buildDataPath)
        {
            ...
            // OpenfortSDK folder in the data folder
            string buildIdentityPath = $"{buildDataPath}/OpenfortSDK/Runtime/OpenfortSDK/";

            // Make sure it exists
            DirectoryInfo buildIdentityInfo = new DirectoryInfo(buildIdentityPath);
            if (!buildIdentityInfo.Exists)
            {
                Directory.CreateDirectory(buildIdentityPath);
            }
            else
            {
                // If the directory exists, clear it
                FileHelpers.ClearDirectory(buildIdentityPath);
            }

            buildIdentityPath = Path.GetFullPath(buildIdentityPath);

            CopyFilesTo(buildIdentityPath);
        }

        private void CopyFilesTo(string destinationPath)
        {
            Debug.Log("Copying OpenfortSDK files...");
            ...
            foreach (string dir in Directory.GetDirectories(openfortSDKWebFilesDir, "*", SearchOption.AllDirectories))
            {
                string dirToCreate = dir.Replace(openfortSDKWebFilesDir, destinationPath);
                Directory.CreateDirectory(dirToCreate);
            }

            foreach (string newPath in Directory.GetFiles(openfortSDKWebFilesDir, "*.*", SearchOption.AllDirectories))
            {
                if (!newPath.EndsWith(".meta"))
                {
                    File.Copy(newPath, newPath.Replace(openfortSDKWebFilesDir, destinationPath), true);
                }
            }
        }
    }
